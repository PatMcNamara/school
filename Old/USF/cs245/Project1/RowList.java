...
/**
 * This is a doubly linked list of rows (which are horizontal ElementLists).
 * Note that any finds should be done through this class as the ColList class
 * does not implement a find method.
 * @author pjmcnamara2
 *
 * @param <E> Type of object contained in the nodes of the Sparse Array (this
 * 		list doesn't actually store any values of type E).
 */
class RowList<E> {
	 ...
	 * either the row where the element is to be added or removed, or (if that
	 * does not exist), it will return the row that would be after the given
	 * row (if it existed).  If the given row is > the last row in the list,
	 * return null.
	 * @param row
	 * @return
	 ...
	 * @author pjmcnamara2
	 */
	private class RIterator extends RowIterator {
