 ...
 * @author pjmcnamara2
 *
 * @param <E> type of object contained by elements of this list (this list
...
class ElementList<E> {
	...
	public E find(int index) {
		return findElementAt(index).value;
	}
	 ...
	 * @return
	 */
	public ElemIterator getIterator() {
	...
	public String toString() {
		...
		ret += "Value = " + value + "\n";
		if(isColumn)
			for(MatrixNode<E> tmp = head; tmp != null; tmp = tmp.nextRow)
				ret += tmp.toString();
		else
			for(MatrixNode<E> tmp = head; tmp != null; tmp = tmp.nextCol)
				ret += tmp.toString();
		return ret;
	}
	 ...
	 * @author pjmcnamara2
	 */
	class EIterator extends ElemIterator {
