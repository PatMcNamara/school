...
{
//  char *file_name = *((char**) (stack_bottom + sizeof(void *) + sizeof(int) + sizeof(char**)));
  char *position;
//  char file_name[14];
  struct intr_frame if_;
  bool success;
//  int stack_size = pg_round_up(stack_bottom) - stack_bottom;
  char *s;
  char *file_name = s = strtok_r(file_name_, " ", &position);
  ...
  if(success) {
    /* Create temp arg list on esp's page top */
...
void *parse_args (char **plist_end, void *esp) {
  char **tmp;
  int len = (unsigned) plist_end - (unsigned) pg_round_down(plist_end);

  /* Word align arg list */
  esp -= ((unsigned int) esp) % WORD_SIZE;//TODO is there already a define for this? //TODO is cast needed?
  ...
  /* Push argv */
  esp -= sizeof(char **);
  tmp = (char**) esp;//TODO might not need to use tmp, instead just cast and dereference
  *tmp = esp + sizeof(char **);
...
/*void process_terminate (void) {//TODO this really shouldn't be necissary, it could instead be merged into thread_exit or process_exit
  thread_current()->exit_controler->value = -1;
  sema_up(&thread_current()->exit_controler->parent);
//  syscall_exit(-1);
//  thread_exit();
}*/
