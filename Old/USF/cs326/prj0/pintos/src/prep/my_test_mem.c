 ...
 *
 * Last updated for CS3204 Spring 2007.
 */
...
basic_tests()
{
  const size_t HEADER = 12;     // would normally used sizeof(free_block)
  ...
  ASSERT(bottom || !!!"Unable to allocate");
  void *medium = mem_alloc(MEMSIZE-HEADER-2*1024);
  ASSERT(medium || !!!"Unable to allocate");
