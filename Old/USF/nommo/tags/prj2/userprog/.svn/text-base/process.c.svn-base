...
process_wait (tid_t child_tid)
{
  ...
  /* Wait for child to exit and then free the exit_struct's resources. */
  if(exit != NULL){
...
process_exit (void)
{
  ...
  for(i = 2; i < 128 && cur->open_files[i] != NULL; i++)
    syscall_close(i);
...
static void *parse_args (char **plist_end, void *esp) {
  char **tmp;
...
load (const char *file_name, void (**eip) (void), void **esp)
{
  ...
  *eip = (void (*) (void)) ehdr.e_entry;
  ...
  /* Leave file open and deny writes to it.  It will be closed by process_exit. */
