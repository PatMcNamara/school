...
void pagedir_set_writeable(uint32_t *pd, const void *vpage, bool writeable) {
  uint32_t *pte = lookup_page (pd, vpage, false);
  if (pte != NULL)
    {
      if (writeable)
...
void pagedir_print_map(uint32_t *pd) {
  int page_tables = 0,
      pages = 0;
  void *addr = 0;
  ...
  for(uint32_t *pde = pd; pde < pd + pd_no (PHYS_BASE); pde++) {
    if(*pde & PTE_P) {
      page_tables++;
      uint32_t pt_base = pde_get_pt(*pde);
      for(uint32_t *pte = pde_get_pt(*pde); pte < pt_base + PGSIZE; pte++) {
        if(*pte & PTE_P) {
          pages++;
          printf("vaddr = %p (paddr = %p) is mapped.\n", addr, (*pte & PTE_ADDR));
        }
        addr += PGSIZE;
      }
    } else {
