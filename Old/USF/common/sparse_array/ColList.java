 ...
 * Notice that this class does not contain a method similer to RowList.find(int,int), all
 * finds should be done by the row list.
 * @author pjmcnamara2
 *
 * @param <E> Type of object contained in nodes of the Sparse Array (this list doesn't actually store any values of type E).
...
class ColList<E> {
	...
	/**
	 * Helper method that is used by the add and remove methods.  It returns either the column where the element is to be added or removed, or (if that does not exist), it will return the column that would be after the given column (if it existed).  If the given column is > the last column in the list, return null.
	 * @param col
	 * @return
	 ...
	 * @author pjmcnamara2
	 */
	private class CIterator extends ColumnIterator<E> {
