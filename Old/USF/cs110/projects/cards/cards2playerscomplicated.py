...
def checkCard(a, b):  #a is card value, b tells if it is a rank or suit
    if b == "rank":  #tests rank
        x = 2  #checking to see if rank of card is a number
        while x <= 10:  #Check)  #checks new card
...
#displays your hand
#def display(player, hand, card):#which player one, what type of hand he had, and high card (if necissary), perhaps rephrase as player 1s pir of queens beet player 2s Ace high
    #print "****************************************"
    #print "Player", str(playerWin) "'s", handWin, "beats",  #you are going to have to run the result back through the priority in order to get the proper rank
    #print "****************************************"

#this is temporary until it can get merged with dislay(), does not work with pairs (yet)
def tie(aRank, bRank):
    if aRank > bRank:
        print "Player 1 wins"
    elif aRank < bRank:
...
#get cards ranks and returns highest combination
def getHand():
...
#should go to display from here
...
if aHand < bHand:  #player 1 wins
    ...
    print "Player 2 wins"
else: #tie - you need to add in for pairs (the highest card might not be the pair
    tie(aRank, bRank)
