...
#give number and return suit: input random number (1-4), return name of suit, no side-effect
def numtosuit(number):
    if number == 1:
	return "HEARTS"
    if number == 2:
	return "SPADES"
...
#prints out type of hand you have: input high card (if no high card, input will be "d") and type of hand, no output, prints type of hand you have
def printhand(high, hand):  #high is the high card
	print "********************************"
	if hand == "high card":  #the high card will only print the value of the high card
		print "You have a", high, "high"
	else: print "You have a", hand
	print "********************************"

#get cards ranks and returns highest combination, input player number, output rank and suit of best good card and hand as a number, asks user for cards
def getHand(player):
    #gets ranks and suits
    if player == 2:  #needs to have random numbers for computer player -- using 3 decks for simplicity
	rank1 = random.randint(2, 14)  #gets random rank
	print "First card:", numberToFace(rank1),  #displays face card insted of number
	suit1 = numtosuit(random.randint(1, 4))  #gets a random number and turns it into suit
	print "of", suit1.lower()  #displayes suit on previous line in lower case
	rank2 = random.randint(2, 14)
	print "Second card:", numberToFace(rank2),
	suit2 = numtosuit(random.randint(1, 4))
	print "of", suit2.lower()
	rank3 = random.randint(2, 14)
	print "Third card:", numberToFace(rank3),
	suit3 = numtosuit(random.randint(1, 4))
	print "of", suit3.lower()
    else:  #user prompted suit
        rank1 = card("rank", 1)
        suit1 = card("suit", 1)
        rank2 = card("rank", 2)
        suit2 = card("suit", 2)
	...
	printhand("d","straight flush")  #d used as a placeholder
        return rank3, suit3, 1
    elif pair1 + pair2 == 2:           #3 of a kind
        printhand("d","3 of a kind")
	return rank3, suit3, 2
    elif straight == 1:                #straight
        printhand("d","straight")
	return rank3, suit3, 3
    elif flush == 1:                   #flush
        printhand("d","flush")
	return rank3, suit3, 4
    elif pair1 + pair2 == 1:           #pair, returns card 2 because it is the only cardf used in both pair posibilites
        printhand("d","pair")
	return rank2, suit3, 5
    else:                              #high card
        if rank3 > 10:                 #face cards need to be turned back from intergers
            rank3 = numberToFace(rank3)
	printhand(rank3, "high card")
        return rank3, suit3, 6

#End of functions

print "First person:"
aRank, aSuit, aHand = getHand(1) #a before name means player 1, hand ranked 1-6 with 1 being best
print "\nSecond person:"
bRank, bSuit, bHand = getHand(2) #b is player 2

#Who won - lower hand value wins, should be combined with the display
